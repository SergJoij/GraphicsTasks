...
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QSpinBox, QLabel, QComboBox
...
class Window(QMainWindow):
    def __init__(self, ra, ga, ba):
        ...
        self.list_of_models = ("RGB", "CMYK", "HSV", "HSB")  # В выпадающий список
        ...
        self.change_l1 = QComboBox(self)
        for i in self.list_of_models:  # заполнеие 1 списка
            self.change_l1.addItem(i)
        self.change_l1.move(130, 20)
        self.change_l1.show()
        self.change_l1.currentTextChanged.connect(self.change_list_number)

        self.change_l2 = QComboBox(self)
        for i in self.list_of_models:  # заполнеие 2 списка
            self.change_l2.addItem(i)
            print(self.change_l2.count())
            if i == self.change_l1.currentText():  # чтоб не попало значение из 1 списка
                color_people = self.change_l2.count() - 1
                self.change_l2.view().setRowHidden(color_people, True)
        self.change_l2.setCurrentIndex(1)  # не RGB, а CMYK
        self.change_l2.move(240, 20)
        self.change_l2.show()
        self.change_l2.currentTextChanged.connect(self.change_list_number)
    ...
    def paintEvent(self, event):
        if self.status_of_convert:  # Если RGB -> CMYK
            if not self.status_of_change:  # Если только что не меняли режим...
                colors = ConvertColorsRgbToCmyk(self.r, self.g, self.b)
            ...
            if not self.status_of_change:  # Если только что не меняли режим...
                colors = ConvertColorsCmykToRgb(self.c, self.m, self.y, self.k)
                colors.convert_colors()
    ...
    def change_list_number(self, event):
        sender = self.sender()
        if sender == self.change_l1:  # если меняем систему ИЗ
            current = self.change_l1.currentIndex()
            self.hider_for_combobox(self.change_l2)  # делаем строку видимой в системе В
            self.change_l2.view().setRowHidden(current, True)  # делаем строку невидимой в системе В (чтоб было != ИЗ)

    def hider_for_combobox(self, combobox):
        for i in range(combobox.count()):
            if combobox.view().isRowHidden(i):
